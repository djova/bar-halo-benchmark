...
def distribution(s,eta,bar=True,nphi=128,nj=2048,J=64.,dtmax=.025,end=None,slope=SLOPE,sigma=8.):
    ...
    f=np.broadcast_to(np.exp(-.5*((j-slope*sigma**2)/sigma)**2)/(sigma*np.sqrt(2*np.pi)*2*np.pi),(nphi,nj)).copy()
    ...
    if not bar:
        gates.update(unforced_mean=abs(rows[-1]['physical_mean_change'])<1e-7,
                     unforced_variance=abs(rows[-1]['variance']-(sigma**2+2*D*end))<1e-7)
...
def trajectories(s,eta,seed,n=32768,bar=True,dtmax=.025,end=None,slope=SLOPE,sigma=8.):
    end=endpoint(s) if end is None else end;steps=int(np.ceil(end/dtmax));dt=end/steps
    rng=np.random.default_rng(seed);j=rng.normal(slope*sigma**2,sigma,n);psi=rng.uniform(-np.pi,np.pi,n);initial=j.copy()
...
def main():
    ...
    p.add_argument('--sigma',type=float,default=8.);a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False);start,cpu=time.monotonic(),time.process_time()
    common=dict(s=a.s,eta=a.eta,bar=not a.no_bar,dtmax=a.dt,end=a.end,slope=a.slope,sigma=a.sigma)
