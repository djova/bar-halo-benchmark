...
const exists=new Set();async function walk(path,prefix=''){for(const d of await readdir(path,{withFileTypes:true})){const name=prefix+d.name;if(d.isDirectory())await walk(path+'/'+d.name,name+'/');else exists.add(name)}}await walk('dist');
...
const m=JSON.parse(await readFile('dist/manifest.json','utf8'));for(const a of m.artifacts){const b=await readFile('dist/'+a.path);a.sha256=createHash('sha256').update(b).digest('hex');a.bytes=b.length}await writeFile('dist/manifest.json',JSON.stringify(m,null,2)+'\n');console.log('Built the canonical article and central evidence; historical explorers link to the public site.');
