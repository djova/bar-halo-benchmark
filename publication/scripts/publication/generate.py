...
VERSION='2026-09-24.2'; SCIENCE='cdb30b5b2f350d2f3de6831995b83f281fe2974e'
...
FIGURE_FILES=[f'diagnostics/population-accuracy/{name}.png' for name in ('population-comparison','population-accuracy','estimator-efficiency','population-gradient-explanation')]
...
def fmt(x):
 ...
 return f'{x:.9g}'
def numeric(row,key):
 return '<span data-record="'+row['id']+'" data-field="'+key+'">'+fmt(row[key])+'</span>'
...
def page(file,title,md,script=''):
 ...
 captions=iter(['Original population comparison','All recorded operational decisions','Complete accuracy values','Matched estimator cost','Model registry']) if file=='paper.html' else iter([])
 def wrap_table(m):
...
# Explicit comparisons, exact evidence selections and claim-specific methods.
# These metadata are not inferred from model family: software reproduction and
# estimator efficiency do not compare a noisy physical treatment to its control.
def selected(path,base,indices):
 d=read(path);out=[]
 for i in indices:
  pointer=f'{base}/{i}';v=d
  for k in pointer.split('/')[1:]:v=v[int(k)] if isinstance(v,list) else v[k]
  keys=('id',) if 'id' in v else tuple(k for k in ('s','tau','cutoff','population') if k in v)
  out.append(dict(path=path,json_pointer=pointer,sha256=digest(W/path),expected_identity={k:v[k] for k in keys}))
 return out
meta={
 'POP-01':dict(comparison='Gaussian versus reference halo at the same density normalization, sweep, noise and endpoint',protocols=['methods/validation-allocation-01.json'],evidence=selected('results/populations.json','/rows',[i for i,r in enumerate(pop) if r['population'] in ('gaussian','halo')])),
 'POP-02':dict(comparison='Paired slope-matched exponential minus reference-halo response',protocols=['methods/validation-allocation-01.json','protocols/ACCURACY_PLAN.md'],evidence=selected('diagnostics/population-accuracy/joint-error-assessment.json','/exponential_halo',[i for i,r in enumerate(read('diagnostics/population-accuracy/joint-error-assessment.json')['exponential_halo']) if r['s']==.25]),uncertainty='Eight paired seed batches; pointwise Student-t interval of exponential-minus-halo contraction, paired timestep/window intervals and independent numerical-refinement proxies; no certified sub-0.1% accuracy.'),
 'PA-ACC-01':dict(comparison='Advance full allowance versus frozen 5% target; approximate kernel forecast versus independently evolved reference halo',protocols=['protocols/ACCURACY_PLAN.md','protocols/KERNEL_01.md','protocols/INDEPENDENT_MATRIX_01.md'],evidence=selected('results/accuracy.json','/rows',[i for i,r in enumerate(acc) if r['population'] in ('exponential','gaussian512')])),
 'PA-ACC-02':dict(comparison='Gaussian128 absolute-gradient allowance versus target and independently evolved intrinsic population error',protocols=['protocols/ACCURACY_PLAN.md','protocols/INDEPENDENT_MATRIX_01.md'],evidence=selected('results/accuracy.json','/rows',[i for i,r in enumerate(acc) if r['population']=='gaussian128'])+selected('diagnostics/population-accuracy/conservatism-explanation.json','/rows',range(4)),uncertainty='Shared eight-batch kernel; simultaneous nominal Student-t/Bonferroni envelope plus paired timestep/mesh proxies. Intrinsic errors use paired deterministic refinement changes, not a pointwise sampling CI.'),
 'EST-01':dict(comparison='Raw versus cumulative-remainder estimators on matched paths, weights and accounted workload',physical_control='Each estimator targets the same noise-minus-smooth bar impulse; this is separate from the estimator comparison',protocols=['protocols/ESTIMATOR_COST_02.md'],evidence=selected('results/cost.json','/rows',[i for i,r in enumerate(cost) if r['population'] in ('halo','stress0.015625')]),observable='Raw/remainder cost-times-variance ratio',units='dimensionless efficiency; CPU seconds reported separately'),
 'POP-MARG-01':dict(comparison='Historical operational intervals versus original allowance, then expanded intervals including numerical proxies versus that same allowance',protocols=['protocols/HELDOUT_POPULATIONS_01.md','protocols/HELDOUT_MATRIX_01.md','protocols/ACCURACY_PLAN.md'],evidence=selected('diagnostics/population-accuracy/joint-error-assessment.json','/historical_forecast_assessment',range(24)),uncertainty='Historical paired pointwise prediction-discrepancy intervals and summed numerical-refinement proxies. Original absolute-floor decisions remain unchanged; expanded proxies are not rigorous bounds.'),
 'CART-01':dict(comparison='Predeclared reduced halo-slice forecast versus weighted prescribed-field Cartesian response',physical_control='Matched imposed-noise and smooth responses in each description',protocols=['protocols/HALO_SLICE_PILOT_01.md']),
 'REP-01':dict(comparison='Original versus reproduced arrays under the same inputs, seeds and numerical configuration',physical_control=None,protocols=['protocols/ACCURACY_REPRODUCTION.md'],observable='Number of arrays reported exactly reproduced',units='array count',uncertainty='Deterministic array equality reported by the project under identical seeds/configuration; no sampling interval or new independent physical sample.'),
 'GAL-01':dict(comparison='Paired force-opening settings under the original endpoint sensitivity criterion',physical_control='Galaxy runs with matched initial conditions and other settings',protocols=['protocols/GALAXY_EXPERIMENT_PLAN.md'],observable='Endpoint bar-amplitude sensitivity between matched force-opening runs',units='Dimensionless Fourier amplitude and relative differences as defined in source'),
 'OBS-01':dict(comparison='Descriptive selected-catalogue amplitude distribution; no matched intervention',physical_control=None,protocols=[],protocol_status='Historical catalogue selection, not a frozen prospective experiment',selection_record='data/observational-summary.json#/selection',observable='Selected amplitude sample count and median',units='galaxy count; dimensionless cosine amplitude')}
for c in claims:
 c.update(meta[c['id']]);c.pop('protocol',None)
 c.setdefault('physical_control','Same prescribed dynamics and population without imposed action noise')
 c['reference_control']=c['comparison'] # retained compatibility alias; never generic boilerplate
 c['uncertainty_method']=c['uncertainty'];c['protocol']=c['protocols'][0] if c['protocols'] else None
 if c['id']=='CART-01':
  c['evidence']=[dict(path=c['outcome'],json_pointer='/'+k,sha256=digest(W/c['outcome'])) for k in ('targets','conditional_decision','independent_3d_validation','cost')]
 if c['id']=='GAL-01':c['evidence']=selected(c['outcome'],'/force_pairs',range(4))
...
assessment_label=lambda code: {'supported':'independently within 5%','contradicted':'independently outside 5%','inconclusive':'independent accuracy inconclusive','unqualified_numerics':'independent numerical qualification unavailable'}.get(code,code)
staticrows=''.join('<tr>'+''.join('<td>'+html.escape(str(v))+'</td>' for v in [f'{LABELS[r["population"]]} / T{int(r["time"])} / {r["cutoff"]}', 'Qualified' if r['qualified_5_percent'] else 'Not qualified',assessment_label(r['independent_assessment']),r['sign_assessment'].replace('_',' ')])+'</tr>' for r in acc)
...
# The article already introduces this condition; retain controls and every value.
explorer=re.sub(r'(<section id="accuracy-explorer">).*?(<p id="accuracy-load")', r'\1<h3>Inspect a recorded comparison</h3>\2', explorer, flags=re.S)
...
pop_table=table(['Population','T','Response','95% lower','95% upper','Numerical/window qualified'],[[LABELS[r['population']],r['time'],numeric(r,'response'),numeric(r,'ci95_low'),numeric(r,'ci95_high'),r['numerical_window_qualified']]for r in pop])
acc_table=table(['ID / population · T · window','Kernel forecast','Independent response','Intrinsic error ± proxy','Forecast − independent halo','Full allowance / 5% target','Advance 5% / independent'],[[f"<a id=\"{r['id']}\" href=\"paper.html?population={r['population']}&amp;time={int(r['time'])}&amp;window={r['cutoff']}#interactive-decision\">{LABELS[r['population']]} · {int(r['time'])} · {r['cutoff']}</a>"+(' (reference)'if r['self_reference']else''),numeric(r,'forecast'),numeric(r,'independent_response'),numeric(r,'intrinsic_population_error')+' ± '+numeric(r,'intrinsic_error_proxy'),numeric(r,'forecast_minus_independent_halo'),numeric(r,'full_allowance')+' / '+numeric(r,'target'),('Qualified'if r['qualified_5_percent']else'Not qualified')+' / '+assessment_label(r['independent_assessment'])]for r in acc])
...
page('paper.html','Population dependence of angular-momentum transfer at a noisy sweeping resonance',article,'<script type="module" src="population-accuracy.js"></script>')
...
datasets.append(dataset('DS-GRAD','data/population-response/publication.json','Recorded population gradients, primitive response and cumulative contributions for the explanation figure',{'action':'512 dimensionless display-cell centres','cumulative_action':'513 ordered display edges','records':'Population/sweep/endpoint selections with gradients and paired estimates','records[].contribution':'Fine-cell gradient products summed before display averaging; mean and pointwise95% interval','records[].cumulative':'Cumulative signed gradient integrals including full population taper','kernels':'Primitive kernel mean and pointwise95% intervals','display_scope':'Coarsening and interpretation; terms are not literal orbital cohorts'},seeds=list(range(94501,94509))))
...
```sh
python3 verify_publication.py --origin https://djova.ca/galaxy-bar/ --expected-release {VERSION}
```
...
- [Complete tagged article]({MIRROR}paper.md) · [GitHub rendered article]({BROWSE}paper.md)
- [Learning guide]({MIRROR}learn.md) · [claims]({MIRROR}claims.json) · [manifest]({MIRROR}manifest.json)
- [Population values]({MIRROR}results/populations.json) · [accuracy operands and labels]({MIRROR}results/accuracy.json) · [estimator costs]({MIRROR}results/cost.json)
...
files+=FIGURE_FILES+['data/population-response/publication.json','diagnostics/population-accuracy/population-gradient-explanation.svg','diagnostics/population-accuracy/gradient-figure-receipt.json']
files+=['communication.html']+sorted(p.relative_to(W).as_posix() for p in (W/'diagnostics/communication').rglob('*') if p.is_file() and '2026-09-24' in str(p))
