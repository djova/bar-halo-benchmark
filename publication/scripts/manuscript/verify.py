...
R=Path(__file__).resolve().parents[2];D=R/'web/downloads/mnras-2026-09-24.2';B=R/'build/mnras-2026-09-24.2'
...
assert all(k in (B/'galaxy-bar-mnras.bbl').read_text()for k in ['Hamilton2023','Chiba2023','Elbers2021','OgilvieLubow2006','GalaxyBar2026','Dattathri2026'])
assert 'REFERENCES'in text and 'APPENDIX A'in text and 'APPENDIX B'in text
assert 'These experiments do not test CDM against SIDM.'in re.sub(r'\s+',' ',text)
...
result={'export':m['export'],'scientific_release':m['scientific_release'],'pages':int(re.search(r'Pages:\s+(\d+)',info)[1]),'figures':3,'bibliography_entries':6,'table_rows':[6,24,24,24,24],'source_and_artifact_hashes_match':True,'unresolved_references':False,'overflowing_tex_boxes':False,'science_rerun':False,'limitations':'Checks export fidelity and layout diagnostics, not numerical error coverage or astrophysical validity.'}
(R/'results/writing-revision').mkdir(parents=True,exist_ok=True)
(R/'results/writing-revision/document-check.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
