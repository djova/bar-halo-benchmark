...
The accumulated bar impulse \(B\) excludes both \(-sT\) and the Brownian impulse; the path budget checks all three separately. Our observable is \textbf{noise minus smooth accumulated bar-mediated transfer}. Positive means more transfer from the imposed bar to the sampled slice. A negative contrast can arise when both treatments transfer angular momentum to the halo but the noisy one transfers less; it does not by itself imply that the bar gains angular momentum.
...
\section{A failed Gaussian approximation and a successful exponential}\label{population-result}
...
\caption{F-POP-01. Population selection changes the early sign and late magnitude of the noise-minus-smooth accumulated bar transfer. Only the initial weight changes: s = 0.25, \(\eta\) = 0.1, central density, initial slope and cutoff 40 are shared. Values are dimensionless \(\int wK_B\,\mathrm dx\), conditional on one fast-action slice. The two endpoints are recorded measurements, not a resolved time history. Intervals are pointwise 95\% Student-t intervals across eight independent trajectory batches; paired numerical controls are assessed separately. The early signs survive the declared timestep/window allowances. \href{https://djova.ca/galaxy-bar/results/populations.json}{Complete values and numerical controls}; \href{https://djova.ca/galaxy-bar/population-response.html\#population-explorer}{interactive weighting}. Analysis version and checksums: \href{https://djova.ca/galaxy-bar/datasets.json}{DS-POP}.}\label{F-POP-01}
\end{figure*}
...
The exponential's point differences from the halo are about 0.050\% and 0.068\%. These close means do not certify sub-0.1\% accuracy: the relevant uncertainty is that of the paired difference, together with numerical sensitivity. The \href{https://djova.ca/galaxy-bar/diagnostics/population-accuracy/joint-error-assessment.json}{difference record} retains that assessment.
...
\section{Separating orbital response from population weighting}\label{kernel}
...
At \(T=10\), the Gaussian's gradient-coordinate contributions inside and outside \(|x|=4\) are approximately \(-0.011058+0.012565=+0.001508\). For the halo they are \(-0.004835+0.000934=-0.003901\). The positive term outweighs the negative one for the Gaussian, while the negative term dominates for the halo. This decomposition explains the sign difference. \textbf{The terms are contributions to the gradient integral, not two literal cohorts of initial orbits.} The \href{https://djova.ca/galaxy-bar/population-response.html\#population-explorer}{weighting explorer} shows the populations, kernel and cumulative cancellation together.
...
\section{When can an approximate population be used?}\label{accuracy}
...
\caption{F-ACC-01. The allowance identifies eight acceptable approximations and conservatively rejects four others that also meet the target. The twenty candidates share s = 0.5 and \(\eta\) = 0.1; their population, endpoint and window vary. Orange circles show full forecast allowances; blue crosses show approximate-kernel forecast discrepancy against the independently evolved halo, including its numerical proxy. Both are divided by the frozen absolute 5\% target. The discrepancy includes kernel-estimation error and is distinct from \textbf{intrinsic population error}, R{[}v{]} − R{[}h{]}, measured by paired independent evolution. The four halo self-references are excluded from candidate counts. Table \ref{tab-acc} · \href{https://djova.ca/galaxy-bar/datasets.json}{DS-ACC metadata}.}\label{F-ACC-01}
\end{figure*}
...
\section{Measuring a small net transfer without large cancellation noise}\label{estimator}
...
\section{Domain of validity and unresolved extensions}\label{limitations}
...
This PDF export, 2026-09-24.2, typesets the scientific article 2026-09-24.1. It carries the editorial revision in both media; the numerical evidence and historical criteria are unchanged. Earlier releases remain archived. The \href{https://djova.ca/galaxy-bar/paper.html}{interactive article} provides the original navigation, selection controls and versioned claim registry.
