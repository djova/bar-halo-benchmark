...
def main():
    ...
    external = spec.get('external_sources', {})
    assert set(external).issubset(sources)
    def source_path(name):
        return Path(external[name]) if name in external else ROOT/name
    ...
    while pending or active:
        ...
        while pending and len(active) < spec['workers']:
            c = pending.pop(0)
            for path, expected in sources.items():
                if sha(source_path(path)) != expected:
