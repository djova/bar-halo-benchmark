"""Full Cartesian orbits with explicitly imposed canonical Js Brownian noise."""
import os
...
from orbit_kernel import Kernel
from characteristics import advance as reduced_advance

def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
def main():
 p=argparse.ArgumentParser();p.add_argument('--out',required=True,type=Path);p.add_argument('--agama-path',type=Path,default=Path('vendor/Agama'));p.add_argument('--pilot',action='store_true');p.add_argument('--pilot-end',type=float,choices=[40.,12720.],default=40.);p.add_argument('--forecast',type=Path);p.add_argument('--case',choices=['A','B'],default='A');p.add_argument('--seed',type=int,required=True);p.add_argument('--n',type=int,default=4096);p.add_argument('--start',type=int,default=0);p.add_argument('--dt',type=float,default=.01);p.add_argument('--noise-refine',type=int,choices=[1,2],default=1);p.add_argument('--no-noise',action='store_true');p.add_argument('--no-bar',action='store_true');a=p.parse_args()
 ...
 pot=agama.Potential(type='Isochrone',mass=1,scaleRadius=.5);am=agama.ActionMapper(pot);af=agama.ActionFinder(pot);kernel=Kernel(ROOT/'build/noise-sweep/orbit-baseline.so')
 ...
 for block in range(blocks+1):
  ...
  leaves=np.random.default_rng(np.random.SeedSequence([a.seed,2,block])).normal(size=(a.start+n,2))[a.start:].T;increments=[np.sqrt(D*h)*(leaves[0]+leaves[1])] if a.noise_refine==1 else [np.sqrt(D*h)*leaves[0],np.sqrt(D*h)*leaves[1]]
